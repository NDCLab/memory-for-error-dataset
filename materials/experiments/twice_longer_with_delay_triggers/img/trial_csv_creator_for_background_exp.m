...
% This script produces the csv files for the alternative experiment that
% has background face. One half of the experiment has straight looking face
% in the background while arrows flanking and the other half will have the
% background face, but we will have faces flanking not arrows.
% 3-5 mins blocks for the surprise. 
...
main_dir = '/Users/khoss005/Library/CloudStorage/OneDrive-FloridaInternationalUniversity/Projects/Psychopy/mfeFlanker/img';
...
for jaguar=1:13 % 12 of these files will be for the main blocks. The last one with 20 trials will be the practice block.
    if jaguar==13
        ...
        fileName = append("alt_trialTable_practice",".csv");
    ...
    elseif jaguar <= 6 % The first 6 files will be used for the arrow flanker task in the alternative task.
        ...
        for zebra=1:length(rightCong)
            ...
            rightCong(zebra,6)= 'right'; %target
            rightCong(zebra,7)= '[0,0]'; % Center image Location on the screen for the Psychopy
            rightCong(zebra,8)= '[.03,0]'; % Right image Location on the screen for the Psychopy
            rightCong(zebra,9)= '[-.03,0]'; % Left image Location on the screen for the Psychopy
            % in the lines below, I am creating a column consisting the
            % straight looking faces for the background.
            nav_temp = strrep(nav_temp1(zebra),'_45','_0');
            rightCong(zebra,10)= nav_temp; % Straight_face
            rightCong(zebra,11)= '[.02, .02]'; % Size for the image
        ...
        for zebra2=1:length(rightIncong)
            ...
            rightIncong(zebra2,8)= '[.03,0]'; % Right image Location on the screen for the Psychopy
            rightIncong(zebra2,9)= '[-.03,0]'; % Left image Location on the screen for the Psychopy
            nav_temp = strrep(nav_temp2(zebra2),'_45','_0');
            rightIncong(zebra2,10)= nav_temp; % Straight_face
            rightIncong(zebra2,11)= '[.02, .02]'; % Size for the image
        end
        ...
        for zebra3=1:length(leftCong)
            ...
            leftCong(zebra3,6)= 'left'; %target
            leftCong(zebra3,7)= '[0,0]'; % Center image Location on the screen for the Psychopy
            leftCong(zebra3,8)= '[.03,0]'; % Right image Location on the screen for the Psychopy
            leftCong(zebra3,9)= '[-.03,0]'; % Left image Location on the screen for the Psychopy
            nav_temp = strrep(nav_temp3(zebra3),'_-45','_0');
            leftCong(zebra3,10)= nav_temp; % Straight_face
            leftCong(zebra3,11)= '[.02, .02]'; % Size for the image
        end
        leftIncong = ~contains(leftDir_faces, leftCong_intact, 'IgnoreCase',true);
        leftIncong = leftDir_faces(leftIncong); % the target faces that will be incongruent.
        ...
        for zebra4=1:length(leftIncong)
            ...
            leftIncong(zebra4,8)= '[.03,0]'; % Right image Location on the screen for the Psychopy
            leftIncong(zebra4,9)= '[-.03,0]'; % Left image Location on the screen for the Psychopy
            nav_temp = strrep(nav_temp4(zebra4),'_-45','_0');
            leftIncong(zebra4,10)= nav_temp; % Straight_face
            leftIncong(zebra4,11)= '[.02, .02]'; % Size for the image
        end
        % Creating the main table that contains all we have created above for the 1st CSV file.
        ...
        fileName = append("alt_trialTable_",string(jaguar),".csv");
        writetable(mainTable, fileName)
        % let's update faces for the next round of this loop. So, it will not
        % have the 32 faces used in this loop.
        facesTemp = ~contains(faces, firstDat_intact, 'IgnoreCase',true);
        faces = faces(facesTemp);
    elseif jaguar >=7 && jaguar <=12
        firstDat = randsample(faces, 32); % each block will have 32 trials.
        firstDat_intact = firstDat; % we need this for the 2nd csv file.
        % we need to randomly select half of the faces to be right directed and the
        % remaining half will be left-directed.
        ...
        for zebra=1:length(rightCong)
            ...
            rightCong(zebra,8)= '[.18,0]'; % Right image Location on the screen for the Psychopy
            rightCong(zebra,9)= '[-.18,0]'; % Left image Location on the screen for the Psychopy
            nav_temp = strrep(rightCong(zebra),'_45','_0');
            rightCong(zebra,10)= nav_temp; % Straight_face
            rightCong(zebra,11)= '[.18, .18]'; % Size for the image
        ...
        for zebra2=1:length(rightIncong)
            ...
            rightIncong(zebra2,8)= '[.18,0]'; % Right image Location on the screen for the Psychopy
            rightIncong(zebra2,9)= '[-.18,0]'; % Left image Location on the screen for the Psychopy
            nav_temp = strrep(rightIncong(zebra2),'_45','_0');
            rightIncong(zebra2,10)= nav_temp; % Straight_face
            rightIncong(zebra2,11)= '[.18, .18]'; % Size for the image
        ...
        for zebra3=1:length(leftCong)
            ...
            leftCong(zebra3,8)= '[.18,0]'; % Right image Location on the screen for the Psychopy
            leftCong(zebra3,9)= '[-.18,0]'; % Left image Location on the screen for the Psychopy
            nav_temp = strrep(leftCong(zebra3),'_-45','_0');
            leftCong(zebra3,10)= nav_temp; % Straight_face
            leftCong(zebra3,11)= '[.18, .18]'; % Size for the image
        end
        leftIncong = ~contains(leftDir_faces, leftCong_intact, 'IgnoreCase',true);
        leftIncong = leftDir_faces(leftIncong); % the target faces that will be incongruent.
        % Let's create left-directed incongruent rows.
        for zebra4=1:length(leftIncong)
            ...
            leftIncong(zebra4,8)= '[.18,0]'; % Right image Location on the screen for the Psychopy
            leftIncong(zebra4,9)= '[-.18,0]'; % Left image Location on the screen for the Psychopy
            nav_temp = strrep(leftIncong(zebra4),'_-45','_0');
            leftIncong(zebra4,10)= nav_temp; % Straight_face
            leftIncong(zebra4,11)= '[.18, .18]'; % Size for the image
        end
        % Creating the main table that contains all we have created above for the 1st CSV file.
        mainTable = table([rightCong(:,1);rightIncong(:,1);leftCong(:,1);leftIncong(:,1)],[rightCong(:,2);rightIncong(:,2);leftCong(:,2);leftIncong(:,2)],[rightCong(:,3);rightIncong(:,3);leftCong(:,3);leftIncong(:,3)],[rightCong(:,4);rightIncong(:,4);leftCong(:,4);leftIncong(:,4)],[rightCong(:,5);rightIncong(:,5);leftCong(:,5);leftIncong(:,5)],[rightCong(:,6);rightIncong(:,6);leftCong(:,6);leftIncong(:,6)],[rightCong(:,7);rightIncong(:,7);leftCong(:,7);leftIncong(:,7)],[rightCong(:,8);rightIncong(:,8);leftCong(:,8);leftIncong(:,8)],[rightCong(:,9);rightIncong(:,9);leftCong(:,9);leftIncong(:,9)],[rightCong(:,10);rightIncong(:,10);leftCong(:,10);leftIncong(:,10)],[rightCong(:,11);rightIncong(:,11);leftCong(:,11);leftIncong(:,11)]);
        mainTable = table2array(mainTable);
...
writetable(surpriseTable(1:height(surpriseTable)/2,:), "alt_surpriseTable1.csv")
writetable(surpriseTable((height(surpriseTable)/2)+1:end,:), "alt_surpriseTable2.csv")
