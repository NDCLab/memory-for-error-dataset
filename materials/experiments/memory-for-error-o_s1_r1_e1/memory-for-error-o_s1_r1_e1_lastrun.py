...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/kihossei/Documents/GitHub/memory-for-error-dataset/materials/experiments/memory-for-error-o_s1_r1_e1/memory-for-error-o_s1_r1_e1_lastrun.py',
...
while continueRoutine:
    ...
    if welcome_keyResp.status == STARTED and not waitOnFlip:
        theseKeys = welcome_keyResp.getKeys(keyList=['right'], waitRelease=False)
...
while continueRoutine:
    ...
    if insructRight_keyResp.status == STARTED and not waitOnFlip:
        theseKeys = insructRight_keyResp.getKeys(keyList=['right'], waitRelease=False)
...
while continueRoutine:
    ...
    if instructLeft_keyResp.status == STARTED and not waitOnFlip:
        theseKeys = instructLeft_keyResp.getKeys(keyList=['left'], waitRelease=False)
...
while continueRoutine:
    ...
    if insructInconRight_keyResp.status == STARTED and not waitOnFlip:
        theseKeys = insructInconRight_keyResp.getKeys(keyList=['right'], waitRelease=False)
...
while continueRoutine:
    ...
    if instructInconLeft_keyResp.status == STARTED and not waitOnFlip:
        theseKeys = instructInconLeft_keyResp.getKeys(keyList=['left'], waitRelease=False)
...
for thisPrac_block_loop in prac_block_loop:
    ...
    while continueRoutine:
        ...
        if prac_reminder_keyResp.status == STARTED and not waitOnFlip:
            theseKeys = prac_reminder_keyResp.getKeys(keyList=['right'], waitRelease=False)
    ...
    for thisPrac_trial_loop in prac_trial_loop:
        ...
        while continueRoutine:
            ...
            if prac_stim_keyResp.status == STARTED and not waitOnFlip:
                theseKeys = prac_stim_keyResp.getKeys(keyList=['left','right'], waitRelease=False)
        ...
        if prac_stim_keyResp.keys: #if at least one response was made this trial
            if prac_stim_keyResp.keys[0] == 'left': #if the FIRST button pressed was a '1'
            ...
            elif prac_stim_keyResp.keys[0] == 'right': #if the FIRST button pressed was a '8'
    ...
    while continueRoutine:
        ...
        if prac_blockFeed_keyResp.status == STARTED and not waitOnFlip:
            theseKeys = prac_blockFeed_keyResp.getKeys(keyList=['right'], waitRelease=False)
...
for thisTask_block_loop in task_block_loop:
    ...
    while continueRoutine:
        ...
        if task_blockReminders_keyResp.status == STARTED and not waitOnFlip:
            theseKeys = task_blockReminders_keyResp.getKeys(keyList=['right'], waitRelease=False)
    ...
    for thisTask_trial_loop in task_trial_loop:
        ...
        while continueRoutine:
            ...
            if task1_stim_keyResp.status == STARTED and not waitOnFlip:
                theseKeys = task1_stim_keyResp.getKeys(keyList=['left','right'], waitRelease=False)
        ...
        if task1_stim_keyResp.keys: #if at least one response was made this trial
            if task1_stim_keyResp.keys[0] == 'left': #if the FIRST button pressed was a '1'
                if target == 'left': #if a left target stim was shown this trial
                    accuracy = 1 #mark this trial as correct
                    numCorr = numCorr +1 #iterate number of correct responses for this block
                elif target == 'right': #if a right target stim was shown this trial
                    accuracy = 0 #mark this trial as an error
            elif task1_stim_keyResp.keys[0] == 'right': #if the FIRST button pressed was a '8'
                if target == 'right': #if a right target stim was shown this trial
                    accuracy = 1 #mark this trial as correct
                    numCorr = numCorr +1 #iterate number of correct responses for this block
                elif target == 'left': #if a left target stim was shown this trial
    ...
    while continueRoutine:
        ...
        if task_blockFeed_keyResp.status == STARTED and not waitOnFlip:
            theseKeys = task_blockFeed_keyResp.getKeys(keyList=['right'], waitRelease=False)
...
while continueRoutine:
    ...
    if errorN_key_resp_2.status == STARTED and not waitOnFlip:
        theseKeys = errorN_key_resp_2.getKeys(keyList=['c'], waitRelease=False)
...
while continueRoutine:
    ...
    if botherRate_key_resp.status == STARTED and not waitOnFlip:
        theseKeys = botherRate_key_resp.getKeys(keyList=['c'], waitRelease=False)
...
while continueRoutine:
    ...
    if instruct_surp1_key_resp.status == STARTED and not waitOnFlip:
        theseKeys = instruct_surp1_key_resp.getKeys(keyList=['right'], waitRelease=False)
...
for thisSurprise_block_loop in surprise_block_loop:
    ...
    while continueRoutine:
        ...
        if instructMainTask_keyResp.status == STARTED and not waitOnFlip:
            theseKeys = instructMainTask_keyResp.getKeys(keyList=['right'], waitRelease=False)
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            if surprise_key_resp.status == STARTED and not waitOnFlip:
                theseKeys = surprise_key_resp.getKeys(keyList=['left','right'], waitRelease=False)
...
while continueRoutine:
    ...
    if instruct_surp1_key_resp_2.status == STARTED and not waitOnFlip:
        theseKeys = instruct_surp1_key_resp_2.getKeys(keyList=['right'], waitRelease=False)
...
for thisFriendly_block_loop in friendly_block_loop:
    ...
    while continueRoutine:
        ...
        if instructMainTask_keyResp_2.status == STARTED and not waitOnFlip:
            theseKeys = instructMainTask_keyResp_2.getKeys(keyList=['right'], waitRelease=False)
    ...
    for thisTrial_2 in trials_2:
        ...
        while continueRoutine:
            ...
            if friendly_key_resp.status == STARTED and not waitOnFlip:
                theseKeys = friendly_key_resp.getKeys(keyList=['left','right'], waitRelease=False)
