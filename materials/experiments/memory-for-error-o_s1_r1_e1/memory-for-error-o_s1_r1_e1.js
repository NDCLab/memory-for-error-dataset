...
function instructRightRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of instructRightComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function instructLeftRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of instructLeftComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function instructInconRightRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of instructInconRightComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function instructInconLeftRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of instructInconLeftComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_trial_loopLoopBegin(prac_trial_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisPrac_trial_loop of prac_trial_loop) {
...
function task_block_loopLoopBegin(task_block_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisTask_block_loop of task_block_loop) {
...
function task_trial_loopLoopBegin(task_trial_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisTask_trial_loop of task_trial_loop) {
...
function surprise_block_loopLoopBegin(surprise_block_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisSurprise_block_loop of surprise_block_loop) {
...
function friendly_block_loopLoopBegin(friendly_block_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisFriendly_block_loop of friendly_block_loop) {
...
function prac_blockRemindersRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_blockRemindersComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function initFixationRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of initFixationComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_stimRoutineRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_stimRoutineComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function prac_blockFeedRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of prac_blockFeedComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function task_blockRemindersRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of task_blockRemindersComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function task_stimRoutineRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of task_stimRoutineComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function task_blockFeedRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of task_blockFeedComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function errorNumbers_2RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of errorNumbers_2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function botherRateRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of botherRateComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function askExperimenterRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of askExperimenterComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function surpriseInstructRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of surpriseInstructComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function instructSurpriseTask2_2RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of instructSurpriseTask2_2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function surpriseTaskRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of surpriseTaskComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function friendlyInstruct1RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of friendlyInstruct1Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function friendlyInstruct2RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of friendlyInstruct2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function friendlyTaskRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of friendlyTaskComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function finishMessageRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of finishMessageComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
