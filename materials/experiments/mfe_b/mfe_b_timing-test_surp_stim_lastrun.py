...
expName = 'mfe_b'  # from the Builder filename that created this script
expInfo = {
    'id': '',
    'cb': ['A', 'B'],
}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\NDClab\\Desktop\\Experiments\\mfe_b\\mfe_b_timing-test_surp_stim_lastrun.py',
...
instruct_surprise1_5 = visual.TextStim(win=win, name='instruct_surprise1_5',
    text='For each face displayed, you must indicate whether you saw it in the previous game (OLD face) or if you did not see it in the previous game (NEW face).\n\nYou have six options: “definitely OLD,” “probably OLD,” “maybe OLD,” “maybe NEW,” “probably NEW,”  or “definitely NEW.”\n\nPractice using the mouse now by selecting a response to continue. ',
    font='Open Sans',
    pos=(0, 0), height=0.03, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
# --- Initialize components for Routine "init_fixation2" ---
...
for thisSurprise_block_loop in surprise_block_loop:
    ...
    # --- Prepare to start Routine "init_fixation2" ---
    continueRoutine = True
    routineForceEnded = False
    # update component parameters for each repeat
    # keep track of which components have finished
    init_fixation2Components = [polygon2]
    for thisComponent in init_fixation2Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        if polygon2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > polygon2.tStartRefresh + 2.0-frameTolerance:
                # keep track of stop time/frame for later
                polygon2.tStop = t  # not accounting for scr refresh
                polygon2.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'polygon2.stopped')
    ...
    for thisTrial in trials:
        ...
        for thisTrial_2 in trials_2:
            ...
            while continueRoutine:
                ...
                if first_surpStimuliA1.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > first_surpStimuliA1.tStartRefresh + 0.5-frameTolerance:
                        # keep track of stop time/frame for later
                        first_surpStimuliA1.tStop = t  # not accounting for scr refresh
                        first_surpStimuliA1.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'first_surpStimuliA1.stopped')
                        first_surpStimuliA1.setAutoDraw(False)
