...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\kihossei\\Documents\\GitHub\\memory-for-error-dataset\\materials\\experiments\\mfeFlanker\\mfe_v1_lastrun.py',
...
surprise_faceStimuli = visual.ImageStim(
    win=win,
    name='surprise_faceStimuli', units='height', 
    image='sin', mask=None, anchor='center',
    ori=0.0, pos=(0, 0), size=(0.3840300018, 0.27),
    color=[1,1,1], colorSpace='rgb', opacity=None,
    flipHoriz=False, flipVert=False,
...
for thisSurprise_block_loop in surprise_block_loop:
    ...
    for thisTrial in trials:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        surprise_faceStimuli.setImage(surpriseFaces)
        topQ.setPos((0, 0.2))
        topQ.setText(topQ_1)
        bottomQ.setPos((0, -0.2))
        bottomQ.setText(bottomQ_2)
        surprise_key_resp.keys = []
        surprise_key_resp.rt = []
        _surprise_key_resp_allKeys = []
        # keep track of which components have finished
        surpriseTaskComponents = [surprise_faceStimuli, topQ, bottomQ, surprise_key_resp]
        ...
        while continueRoutine:
            ...
            # *surprise_faceStimuli* updates
            if surprise_faceStimuli.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                surprise_faceStimuli.frameNStart = frameN  # exact frame index
                surprise_faceStimuli.tStart = t  # local t and not account for scr refresh
                surprise_faceStimuli.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(surprise_faceStimuli, 'tStartRefresh')  # time at next scr refresh
                surprise_faceStimuli.setAutoDraw(True)
        ...
        for thisComponent in surpriseTaskComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        trials.addData('surprise_faceStimuli.started', surprise_faceStimuli.tStartRefresh)
        trials.addData('surprise_faceStimuli.stopped', surprise_faceStimuli.tStopRefresh)
        trials.addData('topQ.started', topQ.tStartRefresh)
        trials.addData('topQ.stopped', topQ.tStopRefresh)
        trials.addData('bottomQ.started', bottomQ.tStartRefresh)
        trials.addData('bottomQ.stopped', bottomQ.tStopRefresh)
        # check responses
