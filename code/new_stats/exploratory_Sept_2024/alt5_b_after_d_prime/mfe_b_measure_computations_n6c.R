...
# This version computes HR after binning the surprise responses and finding a new boundary. After this, I compute HR, etc.
# I  used old flanker as well as new images to plot histograms and find the boundary.
# We exclude subjects based on overall dPrime in this version.
...
proc_fileName <- "processed_data_mfe_b_Proj_n6c.csv" # output filename
...
hist_plot_output_path <- paste(proje_wd, "derivatives", "preprocessed", "psychopy", "alt5_b_histograms", sep ="/", collapse = NULL)
...
main_df <- setNames(data.frame(matrix(ncol = 33, nrow = 0)), c("participant_id", "congAcc", "incongAcc",
                                                               ...
                                                               "num_correctFaces_reported_old", "num_correctFaces_reported_new",
                                                               "overall_hitRate", "overall_false_alarm_rate", "num_removed_fast_trials_in_memory_surp",
                                                               "current_error_HR", "current_correct_HR", "current_diff_HR", "overall_dPrime"))

# write a function that can find the indices of confidence bins between which is the boundary
find_optimal_split <- function(counts) {
  total_sum <- sum(counts)

  # Calculate cumulative sums
  cum_sums <- cumsum(counts)

  # Calculate differences between left and right sums for all possible splits
  differences <- abs(cum_sums - (total_sum - cum_sums))

  # Find the index of the minimum difference
  best_split <- which.min(differences)

  # Return the indices of the optimal split
  return(c(best_split, best_split + 1))
}
...
num_participants_removed_based_overall_dPrime <- 0
        ...
        if (confidency_counts[[1]] < 346 ){
          ...
          if (nrow(legit_error_incong_flankerDat) >= 8 ){
            ...
            if (num_incong_error_faces_in_surp >= 8){
              ...
              # Will be used for histogram and finding the boundary for hit rate computations.
              ...
              for (iii in 1:nrow(surprise_df)){
                if (surprise_df$how_confident_all[iii] == 5){
                  num_Faces_reported_def_old <- num_Faces_reported_def_old + 1
                } else if (surprise_df$how_confident_all[iii] == 3){
                  num_Faces_reported_prob_old <- num_Faces_reported_prob_old + 1
                } else if (surprise_df$how_confident_all[iii] == 1){
                ...
                } else if (surprise_df$how_confident_all[iii] == -5){
                  num_Faces_reported_def_new <- num_Faces_reported_def_new + 1
                }
              }

              # We use these counts to plot histogram for each subject
              hist_counts <- c(num_Faces_reported_def_old, num_Faces_reported_prob_old, num_Faces_reported_may_old,
                          num_Faces_reported_may_new, num_Faces_reported_prob_new, num_Faces_reported_def_new)
              # create the bin labels we want to display on the plot
              bin_labels <- c("Def. Old", "Prob. Old", "May. Old", "May. New", "Prob. New", "Def. New")

              # Create a temporary data frame for histogram
              hist_df <- data.frame(
              ...
              hist_title <- paste("Histogram of Participant ", participant_id)
              hist_plot <- ggplot(hist_df, aes(x = bin, y = count)) +
              ...
              # Save this participant's histogram
              ggsave(filename = paste0('hist_sub-',participant_id,'.png',collapse = NULL), plot = hist_plot, path = hist_plot_output_path, device = "png", width = 10, height = 6, dpi = 300)

              # Find the indices between which will be the new boundary
              boundary_indices <- find_optimal_split(hist_counts)
              ...
              num_errorFaces_reported_old <- 0 # this is the number of incongruent error faces that they report as definitely OLD and will be updated in the loop below:
              num_errorFaces_reported_new <- 0
              ...
              for (iii in 1:nrow(legit_error_incong_flankerDat)){
                ...
                if (nrow(temp_face_row_in_surp) == 1){ # if old incong error face exist in the surprise_df
                  if (temp_face_row_in_surp$how_confident_all == 5){ # the face is identified as definitely old
                    bin_index <- 1
                    if (bin_index <= boundary_indices[1]){
                      num_errorFaces_reported_old <- num_errorFaces_reported_old + 1
                    } else if (bin_index > boundary_indices[1]){
                      num_errorFaces_reported_new <- num_errorFaces_reported_new + 1
                    }
              ...
              num_correctFaces_reported_old <- 0 # this is the number of incongruent correct faces that they report as definitely OLD and will be updated in the loop below:
              num_correctFaces_reported_new <- 0
              ...
              for (iii in 1:nrow(legit_correct_incong_flankerDat)){
                ...
                if (nrow(temp_face_row_in_surp) == 1){ # if old incong correct face exist in the surprise_df
                  if (temp_face_row_in_surp$how_confident_all == 5){ # the face is identified as definitely old
                    bin_index <- 1
                    if (bin_index <= boundary_indices[1]){
                      num_correctFaces_reported_old <- num_correctFaces_reported_old + 1
                    } else if (bin_index > boundary_indices[1]){
                      num_correctFaces_reported_new <- num_correctFaces_reported_new + 1
                    }
                  } else if (temp_face_row_in_surp$how_confident_all == 3){ # the face is identified as probably old
                    bin_index <- 2
                    if (bin_index <= boundary_indices[1]){
              ...
              # Compute Hit rate (HR)
              current_error_HR <- num_errorFaces_reported_old / (num_errorFaces_reported_old + num_errorFaces_reported_new)
              current_correct_HR <- num_correctFaces_reported_old / (num_correctFaces_reported_old + num_correctFaces_reported_new)
              current_diff_HR <- current_error_HR - current_correct_HR
              ...
              # Exclude based on overall d-prime value. The threshold is 0.2
              if (overall_dPrime > 0.2){
                ...
                main_df[nrow(main_df) + 1,] <-c(participant_id, congAcc, incongAcc,
                                                ...
                                                congErr_meanRT, incongErr_meanRT, congErr_logMeanRT, incongErr_logMeanRT,
                                                incongCorr_logMeanRT, flankEff_meanACC, flankEff_meanRT, flankEff_logMeanRT,
                                                reported_errors, committed_errors, memoryBias_score, number_of_surp_resp_items_used, num_errorFaces_reported_old,
                                                num_errorFaces_reported_new, num_correctFaces_reported_old,
                                                num_correctFaces_reported_new,
                                                overall_hitRate, overall_false_alarm_rate,
                                                num_removed_fast_trials_in_memory_surp, current_error_HR, current_correct_HR,
                                                current_diff_HR, overall_dPrime)


              } else { # If a participant has been excluded because they had less than 0.2 overall d prime, we add 1 to the counter below.
                num_participants_removed_based_overall_dPrime <- num_participants_removed_based_overall_dPrime + 1
                ### Printing output
                print(paste("Participant ", participant_id, " was excluded because they had less than 0.2 overall d prime."))
