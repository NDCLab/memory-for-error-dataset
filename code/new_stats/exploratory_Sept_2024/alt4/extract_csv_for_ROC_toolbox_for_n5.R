...
# THIS version does not have different coditions for error and correct trials. They are all combined,
...
output_for_current_trial_fileName <- "longDat_current_trial_n5_with_headers.csv" # output filename
output_for_post_trial_fileName <- "longDat_post_trial_n5_with_headers.csv" # output filename
...
for (subject in 1:nrow(main_df)){
  ...
  for (i in 1){ # for condition label (all)
    cond_label <- "all"
      ...
      if (ii == 1){
        ...
        for (iii in 1:6){ # for rating bin
          if (iii == 1){ # definitely old
            rating_bin <- 6
            response_freq <- main_df$num_Faces_reported_def_old[subject]
            ...
            rating_bin <- 5
            response_freq <- main_df$num_Faces_reported_prob_old[subject]
            roc_long_dat_current_trial[nrow(roc_long_dat_current_trial) + 1,] <-c(subject_id, group_id, cond_label, item_label, rating_bin, response_freq)
          } else if (iii == 3){ # maybe old
            rating_bin <- 4
            response_freq <- main_df$num_Faces_reported_may_old[subject]
            roc_long_dat_current_trial[nrow(roc_long_dat_current_trial) + 1,] <-c(subject_id, group_id, cond_label, item_label, rating_bin, response_freq)
          } else if (iii == 4){ # maybe new
            rating_bin <- 3
            response_freq <- main_df$num_Faces_reported_may_new[subject]
            roc_long_dat_current_trial[nrow(roc_long_dat_current_trial) + 1,] <-c(subject_id, group_id, cond_label, item_label, rating_bin, response_freq)
          } else if (iii == 5){ # probably new
            rating_bin <- 2
            response_freq <- main_df$num_Faces_reported_prob_new[subject]
            roc_long_dat_current_trial[nrow(roc_long_dat_current_trial) + 1,] <-c(subject_id, group_id, cond_label, item_label, rating_bin, response_freq)
          } else if (iii == 6){ # definitely new
            rating_bin <- 1
            response_freq <- main_df$num_Faces_reported_def_new[subject]
            roc_long_dat_current_trial[nrow(roc_long_dat_current_trial) + 1,] <-c(subject_id, group_id, cond_label, item_label, rating_bin, response_freq)
          }
        }
      } else if (ii == 2){
...
for (subject in 1:nrow(main_df)){
  subject_id <- main_df$participant_id[subject]
  group_id <- main_df$SA_group[subject]
  for (i in 1){ # for condition label
    cond_label <- "all"
    for (ii in 1:2){ # for item labels
      if (ii == 1){
        ...
        for (iii in 1:6){ #for rating bin
          if (iii == 1){
            rating_bin <- 6
            response_freq <- main_df$num_post_Faces_reported_def_old[subject]
            ...
            rating_bin <- 5
            response_freq <- main_df$num_post_Faces_reported_prob_old[subject]
            roc_long_dat_post_trial[nrow(roc_long_dat_post_trial) + 1,] <-c(subject_id, group_id, cond_label, item_label, rating_bin, response_freq)
          } else if (iii == 3){
            rating_bin <- 4
            response_freq <- main_df$num_post_Faces_reported_may_old[subject]
            roc_long_dat_post_trial[nrow(roc_long_dat_post_trial) + 1,] <-c(subject_id, group_id, cond_label, item_label, rating_bin, response_freq)
          } else if (iii == 4){
            rating_bin <- 3
            response_freq <- main_df$num_post_Faces_reported_may_new[subject]
            roc_long_dat_post_trial[nrow(roc_long_dat_post_trial) + 1,] <-c(subject_id, group_id, cond_label, item_label, rating_bin, response_freq)
          } else if (iii == 5){
            rating_bin <- 2
            response_freq <- main_df$num_post_Faces_reported_prob_new[subject]
            roc_long_dat_post_trial[nrow(roc_long_dat_post_trial) + 1,] <-c(subject_id, group_id, cond_label, item_label, rating_bin, response_freq)
          } else if (iii == 6){
            rating_bin <- 1
            response_freq <- main_df$num_post_Faces_reported_def_new[subject]
            roc_long_dat_post_trial[nrow(roc_long_dat_post_trial) + 1,] <-c(subject_id, group_id, cond_label, item_label, rating_bin, response_freq)
          }
        }
      } else if (ii == 2){
