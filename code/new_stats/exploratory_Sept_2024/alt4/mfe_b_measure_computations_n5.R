...
# This version computes the measures of interest by putting the error and correct trials together. I am using both incongruent and congruent trials.
# and only legit flanker trials.
...
proc_fileName <- "processed_data_mfe_b_Proj_n5.csv" # output filename
...
main_df <- setNames(data.frame(matrix(ncol = 43, nrow = 0)), c("participant_id", "congAcc", "incongAcc",
                                                               ...
                                                               "num_Faces_reported_def_old", "num_Faces_reported_prob_old", "num_Faces_reported_may_old",
                                                               "num_Faces_reported_may_new", "num_Faces_reported_prob_new", "num_Faces_reported_def_new",
                                                               ...
                                                               "num_post_Faces_reported_def_old",
                                                               "num_post_Faces_reported_prob_old", "num_post_Faces_reported_may_old", "num_post_Faces_reported_may_new",
                                                               "num_post_Faces_reported_prob_new", "num_post_Faces_reported_def_new",
                                                               "overall_hitRate", "overall_false_alarm_rate", "num_removed_fast_trials_in_memory_surp"))
        ...
        if (confidency_counts[[1]] < 346 ){
          ...
          if (nrow(legit_error_incong_flankerDat) >= 8 ){
            ...
            if (num_incong_error_faces_in_surp >= 8){
              ...
              # creating the dataframe that has legit incongruent and congruent error and correct trials.
              legit_flanker_df <- filter(flanker_df, current_trial_legitResponse == 1)



              ### number of legit faces identified as definitely old
              # how_confident_all -> # -5: Definitely new; -3: Probably new; -1: maybe new;
              #                              5: Definitely old; 3: Probably old; 1: maybe old
              num_Faces_reported_def_old <- 0 # this is the number of incongruent error faces that they report as definitely OLD and will be updated in the loop below:
              num_Faces_reported_prob_old <- 0
              num_Faces_reported_may_old <- 0
              num_Faces_reported_may_new <- 0
              num_Faces_reported_prob_new <- 0
              num_Faces_reported_def_new <- 0

              for (iii in 1:nrow(legit_flanker_df)){
                temp_face_from_flanker <- legit_flanker_df$current_trial_face[iii]
                temp_face_row_in_surp <- filter(surprise_df, face == temp_face_from_flanker)
                if (nrow(temp_face_row_in_surp) == 1){ # if old face exist in the surprise_df
                  if (temp_face_row_in_surp$how_confident_all == 5){ # the face is identified as definitely old
                    num_Faces_reported_def_old <- num_Faces_reported_def_old + 1
                  } else if (temp_face_row_in_surp$how_confident_all == 3){ # the face is identified as probably old
                    num_Faces_reported_prob_old <- num_Faces_reported_prob_old + 1
                  } else if (temp_face_row_in_surp$how_confident_all == 1){ # the face is identified as maybe old
                    num_Faces_reported_may_old <- num_Faces_reported_may_old + 1
                  } else if (temp_face_row_in_surp$how_confident_all == -1){ # the face is identified as maybe new
                    num_Faces_reported_may_new <- num_Faces_reported_may_new + 1
                  } else if (temp_face_row_in_surp$how_confident_all == -3){ # the face is identified as probably new
                    num_Faces_reported_prob_new <- num_Faces_reported_prob_new + 1
                  } else if (temp_face_row_in_surp$how_confident_all == -5){ # the face is identified as definitely new
                    num_Faces_reported_def_new <- num_Faces_reported_def_new + 1
                  }
                }
              ...
              ## POST trials:
              num_post_Faces_reported_def_old <- 0 # this is the number of incongruent post error faces that they report as definitely OLD and will be updated in the loop below:
              num_post_Faces_reported_prob_old <- 0
              num_post_Faces_reported_may_old <- 0
              num_post_Faces_reported_may_new <- 0
              num_post_Faces_reported_prob_new <- 0
              num_post_Faces_reported_def_new <- 0

              legit_post_flankerDat <- filter(legit_flanker_df, post_trial_legitResponse == 1)
              for (iii in 1:nrow(legit_post_flankerDat)){
                temp_face_from_flanker <- legit_post_flankerDat$post_trial_face[iii]
                temp_face_row_in_surp <- filter(surprise_df, face == temp_face_from_flanker)
                if (nrow(temp_face_row_in_surp) == 1){ # if old post face exist in the surprise_df
                  if (temp_face_row_in_surp$how_confident_all == 5){ # the face is identified as definitely old
                    num_post_Faces_reported_def_old <- num_post_Faces_reported_def_old + 1
                  } else if (temp_face_row_in_surp$how_confident_all == 3){ # the face is identified as probably old
                    num_post_Faces_reported_prob_old <- num_post_Faces_reported_prob_old + 1
                  } else if (temp_face_row_in_surp$how_confident_all == 1){ # the face is identified as maybe old
                    num_post_Faces_reported_may_old <- num_post_Faces_reported_may_old + 1
                  } else if (temp_face_row_in_surp$how_confident_all == -1){ # the face is identified as maybe new
                    num_post_Faces_reported_may_new <- num_post_Faces_reported_may_new + 1
                  } else if (temp_face_row_in_surp$how_confident_all == -3){ # the face is identified as probably new
                    num_post_Faces_reported_prob_new <- num_post_Faces_reported_prob_new + 1
                  } else if (temp_face_row_in_surp$how_confident_all == -5){ # the face is identified as definitely new
                    num_post_Faces_reported_def_new <- num_post_Faces_reported_def_new + 1
                  }
                }
              } # closing the loop
              ...
              if (num_post_Faces_reported_def_old + num_post_Faces_reported_prob_old + num_post_Faces_reported_may_old + num_post_Faces_reported_may_new + num_post_Faces_reported_prob_new + num_post_Faces_reported_def_new >= 8){

                # DO NOTHING
              } else if (num_post_Faces_reported_def_old + num_post_Faces_reported_prob_old + num_post_Faces_reported_may_old + num_post_Faces_reported_may_new + num_post_Faces_reported_prob_new + num_post_Faces_reported_def_new < 8){

                num_post_Faces_reported_def_old <- NA
                num_post_Faces_reported_prob_old <- NA
                num_post_Faces_reported_may_old <- NA
                num_post_Faces_reported_may_new <- NA
                num_post_Faces_reported_prob_new <- NA
                num_post_Faces_reported_def_new <- NA
              }
              ...
              main_df[nrow(main_df) + 1,] <-c(participant_id, congAcc, incongAcc,
                                              ...
                                              congErr_meanRT, incongErr_meanRT, congErr_logMeanRT, incongErr_logMeanRT,
                                              incongCorr_logMeanRT, flankEff_meanACC, flankEff_meanRT, flankEff_logMeanRT,
                                              reported_errors, committed_errors, memoryBias_score, number_of_surp_resp_items_used, num_Faces_reported_def_old,
                                              num_Faces_reported_prob_old, num_Faces_reported_may_old, num_Faces_reported_may_new,
                                              num_Faces_reported_prob_new, num_Faces_reported_def_new, num_post_Faces_reported_def_old,
                                              num_post_Faces_reported_prob_old, num_post_Faces_reported_may_old, num_post_Faces_reported_may_new,
                                              num_post_Faces_reported_prob_new, num_post_Faces_reported_def_new, num_foil_faces_reported_def_old,
                                              num_foil_faces_reported_prob_old, num_foil_faces_reported_may_old, num_foil_faces_reported_def_new,
                                              num_foil_faces_reported_prob_new, num_foil_faces_reported_may_new, overall_hitRate, overall_false_alarm_rate,
                                              num_removed_fast_trials_in_memory_surp)
