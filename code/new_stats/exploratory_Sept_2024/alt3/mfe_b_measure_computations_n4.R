...
proc_fileName <- "processed_data_mfe_b_Proj_n4.csv" # output filename
...
main_df <- setNames(data.frame(matrix(ncol = 35, nrow = 0)), c("participant_id", "congAcc", "incongAcc",
                                                               ...
                                                               "overall_hitRate", "overall_false_alarm_rate", "num_errorFaces_reported_def_old", "num_errorFaces_reported_def_new", "num_post_errorFaces_reported_def_old", "num_post_errorFaces_reported_def_new"))
        ...
        if (confidency_counts[[1]] < 346 ){
          ...
          if (nrow(legit_error_incong_flankerDat) >= 8 ){
            ...
            if (num_incong_error_faces_in_surp >= 8){
              ...
              if (num_errorFaces_reported_def_old + num_errorFaces_reported_def_new>= 8){
                current_error_HR <- (num_errorFaces_reported_def_old )/((num_errorFaces_reported_def_old) + (num_errorFaces_reported_def_new))
                current_correct_HR <- (num_correctFaces_reported_def_old)/((num_correctFaces_reported_def_old) + (num_correctFaces_reported_def_new))
                current_HR_diff <- current_error_HR - current_correct_HR
              } else if (num_errorFaces_reported_def_old + num_errorFaces_reported_def_new < 8){
                current_error_HR <- NA
              ...
              if (num_post_errorFaces_reported_def_old + num_post_errorFaces_reported_def_new>= 8){
                post_error_HR <- (num_post_errorFaces_reported_def_old )/((num_post_errorFaces_reported_def_old) + (num_post_errorFaces_reported_def_new))
                post_correct_HR <- (num_post_correctFaces_reported_def_old)/((num_post_correctFaces_reported_def_old) + (num_post_correctFaces_reported_def_new))
                post_HR_diff <- post_error_HR - post_correct_HR
              } else if (num_post_errorFaces_reported_def_old + num_post_errorFaces_reported_def_new < 8){
                post_error_HR <- NA
                post_correct_HR <- NA
                post_HR_diff <- NA
              }
              ...
              main_df[nrow(main_df) + 1,] <-c(participant_id, congAcc, incongAcc,
                                              ...
                                              num_errorFaces_reported_def_old, num_errorFaces_reported_def_new, num_post_errorFaces_reported_def_old, num_post_errorFaces_reported_def_new)
