...
main_df <-  read.csv(file = paste(processed_file_input, "processed_data_mfe_b_Proj_n4.csv", sep ="/", collapse = NULL), stringsAsFactors = FALSE, na.strings=c("", "NA"))
...
t.test(main_df$current_correct_HR, main_df$current_error_HR, paired = TRUE, na.action = na.omit) # sig9, correct is larger
...
ggplot(main_df, aes(x=scaared_b_scrdTotal_s1_r1_e1, y=current_HR_diff)) + geom_point(size = 4) + geom_smooth(method="lm") +
  labs(x = "SCAARED total anxiety score", y = "current_HR_diff") +
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + theme(axis.text = element_text(size = 15)) + theme(text = element_text(size = 18))
current_HR_reg_3 <- lm(current_error_HR ~ scaared_b_scrdTotal_s1_r1_e1, data = main_df)
summary(current_HR_reg_3)
ggplot(main_df, aes(x=scaared_b_scrdTotal_s1_r1_e1, y=current_error_HR)) + geom_point(size = 4) + geom_smooth(method="lm") +
  labs(x = "SCAARED total anxiety score", y = "current_error_HR") +
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + theme(axis.text = element_text(size = 15)) + theme(text = element_text(size = 18))
current_HR_reg_3 <- lm(current_correct_HR ~ scaared_b_scrdTotal_s1_r1_e1, data = main_df)
summary(current_HR_reg_3)
ggplot(main_df, aes(x=scaared_b_scrdTotal_s1_r1_e1, y=current_correct_HR)) + geom_point(size = 4) + geom_smooth(method="lm") +
  labs(x = "SCAARED total anxiety score", y = "current_correct_HR") +
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                     panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + theme(axis.text = element_text(size = 15)) + theme(text = element_text(size = 18))
