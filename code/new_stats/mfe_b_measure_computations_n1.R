...
main_df <- setNames(data.frame(matrix(ncol = 63, nrow = 0)), c("participant_id", "congAcc", "incongAcc",
                                                               ...
                                                               "reported_errors", "committed_errors", "memoryBias_score", "num_tooFast_surpTrials", "number_of_surp_resp_items_used", "num_errorFaces_reported_def_old",
                                                               "num_errorFaces_reported_prob_old", "num_errorFaces_reported_may_old",
                                                               "num_errorFaces_reported_may_new", "num_errorFaces_reported_prob_new", "num_errorFaces_reported_def_new",
                                                               "num_correctFaces_reported_def_old", "num_correctFaces_reported_prob_old", "num_correctFaces_reported_may_old",
                                                               "num_correctFaces_reported_may_new", "num_correctFaces_reported_prob_new", "num_correctFaces_reported_def_new",
                                                               "num_foil_faces_reported_def_old", "num_foil_faces_reported_prob_old", "num_foil_faces_reported_may_old",
                                                               "num_foil_faces_reported_def_new", "num_foil_faces_reported_prob_new", "num_foil_faces_reported_may_new",
                                                               "num_post_errorFaces_reported_def_old",
                                                               "num_post_errorFaces_reported_prob_old", "num_post_errorFaces_reported_may_old", "num_post_errorFaces_reported_may_new",
                                                               "num_post_errorFaces_reported_prob_new", "num_post_errorFaces_reported_def_new", "num_post_correctFaces_reported_def_old",
                                                               "num_post_correctFaces_reported_prob_old", "num_post_correctFaces_reported_may_old", "num_post_correctFaces_reported_may_new",
                                                               "num_post_correctFaces_reported_prob_new", "num_post_correctFaces_reported_def_new",
                                                               "overall_hitRate", "overall_false_alarm_rate", "overall_dPrime", "num_removed_fast_trials_in_memory_surp", "num_images_reported_old_from_just_olds",
                                                               "num_images_reported_new_from_just_olds", "bias_towards_old_from_just_olds", "num_images_reported_old_from_all",
                                                               "num_images_reported_new_from_all", "bias_towards_old_from_all_images"))
        ...
        if (confidency_counts[[1]] < 346 ){
          ...
          if (nrow(legit_error_incong_flankerDat) >= 8 ){
            ...
            if (num_incong_error_faces_in_surp >= 8){
              ...
              ############################# COMPUTE BIAS TOWARDS OLD/NEW #############################
              # We are gonna compute this bias in two ways. One way is to compute whether the participant is biased towards old/new
              # by only considering images from the flanker task (only old images).
              ### I may also compute it separately for errors and corrects. (TBD)

              # The other way is to compute it by considering all the old and new images.
              # 1. Considering only old images in the surp:
              # Create a temp dataframe that has only images from the flanker task.
              tempDat_for_oldNew_bias <- filter(surprise_df, is_new == 0)
              num_images_reported_old_from_just_olds <- nrow(filter(tempDat_for_oldNew_bias, identified_as_new == 0))
              num_images_reported_new_from_just_olds <- nrow(filter(tempDat_for_oldNew_bias, identified_as_new == 1))
              bias_towards_old_from_just_olds <- ((num_images_reported_old_from_just_olds - num_images_reported_new_from_just_olds)/ num_images_reported_old_from_just_olds)
              # 2. Considering all (old + new) images in the surp:
              num_images_reported_old_from_all <- nrow(filter(surprise_df, identified_as_new == 0))
              num_images_reported_new_from_all <- nrow(filter(surprise_df, identified_as_new == 1))
              bias_towards_old_from_all_images <- ((num_images_reported_old_from_all - num_images_reported_new_from_all)/ num_images_reported_old_from_all)


              ############################# END of COMPUTING BIAS TOWARDS OLD/NEW ####################
              ...
              if (num_post_errorFaces_reported_def_old + num_post_errorFaces_reported_prob_old + num_post_errorFaces_reported_may_old + num_post_errorFaces_reported_may_new + num_post_errorFaces_reported_prob_new + num_post_errorFaces_reported_def_new >= 8){

                # DO NOTHING
              } else if (num_post_errorFaces_reported_def_old + num_post_errorFaces_reported_prob_old + num_post_errorFaces_reported_may_old + num_post_errorFaces_reported_may_new + num_post_errorFaces_reported_prob_new + num_post_errorFaces_reported_def_new < 8){

                num_post_errorFaces_reported_def_old <- NA
                num_post_errorFaces_reported_prob_old <- NA
                num_post_errorFaces_reported_may_old <- NA
                num_post_errorFaces_reported_may_new <- NA
                num_post_errorFaces_reported_prob_new <- NA
                num_post_errorFaces_reported_def_new <- NA

                num_post_correctFaces_reported_def_old <- NA
                num_post_correctFaces_reported_prob_old <- NA
                num_post_correctFaces_reported_may_old <- NA
                num_post_correctFaces_reported_may_new <- NA
                num_post_correctFaces_reported_prob_new <- NA
                num_post_correctFaces_reported_def_new <- NA

              }

              num_foil_faces_reported_def_old <- 0
              num_foil_faces_reported_prob_old <- 0
              num_foil_faces_reported_may_old <- 0
              num_foil_faces_reported_def_new <- 0
              num_foil_faces_reported_prob_new <- 0
              num_foil_faces_reported_may_new <- 0
              # surp_response_conf_all -> # -5: Definitely new; -3: Probably new; -1: maybe new;
              #                              5: Definitely old; 3: Probably old; 1: maybe old
              foil_faces_df <- filter(surprise_df, is_new == 1)
              for(ixbz in 1:nrow(foil_faces_df)){
                if (foil_faces_df$how_confident_all[ixbz] == 5){
                  num_foil_faces_reported_def_old <- num_foil_faces_reported_def_old + 1
                } else if (foil_faces_df$how_confident_all[ixbz] == 3){
                  num_foil_faces_reported_prob_old <- num_foil_faces_reported_prob_old + 1
                } else if (foil_faces_df$how_confident_all[ixbz] == 1){
                  num_foil_faces_reported_may_old <- num_foil_faces_reported_may_old + 1
                } else if (foil_faces_df$how_confident_all[ixbz] == -1){
                  num_foil_faces_reported_may_new <- num_foil_faces_reported_may_new + 1
                } else if (foil_faces_df$how_confident_all[ixbz] == -3){
                  num_foil_faces_reported_prob_new <- num_foil_faces_reported_prob_new + 1
                } else if (foil_faces_df$how_confident_all[ixbz] == -5){
                  num_foil_faces_reported_def_new <- num_foil_faces_reported_def_new + 1
                }
              }
              ...
              main_df[nrow(main_df) + 1,] <-c(participant_id, congAcc, incongAcc,
                                              ...
                                              incongCorr_logMeanRT, flankEff_meanACC, flankEff_meanRT, flankEff_logMeanRT,
                                              reported_errors, committed_errors, memoryBias_score, num_tooFast_surpTrials, number_of_surp_resp_items_used, num_errorFaces_reported_def_old,
                                              num_errorFaces_reported_prob_old, num_errorFaces_reported_may_old, num_errorFaces_reported_may_new,
                                              num_errorFaces_reported_prob_new, num_errorFaces_reported_def_new, num_correctFaces_reported_def_old,
                                              num_correctFaces_reported_prob_old, num_correctFaces_reported_may_old, num_correctFaces_reported_may_new,
                                              num_correctFaces_reported_prob_new, num_correctFaces_reported_def_new, num_post_correctFaces_reported_def_old,
                                              num_post_correctFaces_reported_prob_old, num_post_correctFaces_reported_may_old, num_post_correctFaces_reported_may_new,
                                              num_post_correctFaces_reported_prob_new, num_post_correctFaces_reported_def_new, num_post_errorFaces_reported_def_old,
                                              num_post_errorFaces_reported_prob_old, num_post_errorFaces_reported_may_old, num_post_errorFaces_reported_may_new,
                                              num_post_errorFaces_reported_prob_new, num_post_errorFaces_reported_def_new, num_foil_faces_reported_def_old,
                                              num_foil_faces_reported_prob_old, num_foil_faces_reported_may_old, num_foil_faces_reported_def_new,
                                              num_foil_faces_reported_prob_new, num_foil_faces_reported_may_new, overall_hitRate, overall_false_alarm_rate, overall_dPrime,
                                              num_removed_fast_trials_in_memory_surp, num_images_reported_old_from_just_olds, num_images_reported_new_from_just_olds,
                                              bias_towards_old_from_just_olds, num_images_reported_old_from_all, num_images_reported_new_from_all,
                                              bias_towards_old_from_all_images)
