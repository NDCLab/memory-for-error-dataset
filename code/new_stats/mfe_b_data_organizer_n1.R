...
# Last Update: 2024-08-06 (YYYY-MM-DD)
...
for(subject in 1:length(datafiles_list)){
  ...
  if (psychopyDatTrim$cb[1] == "A"){
    for (yyy in 1:nrow(psychopyDatTrim)){
      if (!is.na(psychopyDatTrim$firstStim_sliderA1.response[yyy])){
        if (psychopyDatTrim$firstStim_sliderA1.response[yyy] == 1){
          psychopyDatTrim$surp_response_id[yyy] <- -1
          psychopyDatTrim$surp_response_conf[yyy] <- 5
          psychopyDatTrim$surp_response_conf_all[yyy] <- -5

        } else if (psychopyDatTrim$firstStim_sliderA1.response[yyy] == 2){
          psychopyDatTrim$surp_response_id[yyy] <- -1
          psychopyDatTrim$surp_response_conf[yyy] <- 3
          psychopyDatTrim$surp_response_conf_all[yyy] <- -3

        } else if (psychopyDatTrim$firstStim_sliderA1.response[yyy] == 3){
          psychopyDatTrim$surp_response_id[yyy] <- -1
          psychopyDatTrim$surp_response_conf[yyy] <- 1
          psychopyDatTrim$surp_response_conf_all[yyy] <- -1

        } else if (psychopyDatTrim$firstStim_sliderA1.response[yyy] == 4){
          psychopyDatTrim$surp_response_id[yyy] <- 1
          psychopyDatTrim$surp_response_conf[yyy] <- 1
          psychopyDatTrim$surp_response_conf_all[yyy] <- 1

        } else if (psychopyDatTrim$firstStim_sliderA1.response[yyy] == 5){
          psychopyDatTrim$surp_response_id[yyy] <- 1
          psychopyDatTrim$surp_response_conf[yyy] <- 3
          psychopyDatTrim$surp_response_conf_all[yyy] <- 3

        } else if (psychopyDatTrim$firstStim_sliderA1.response[yyy] == 6){
          psychopyDatTrim$surp_response_id[yyy] <- 1
          psychopyDatTrim$surp_response_conf[yyy] <- 5
          psychopyDatTrim$surp_response_conf_all[yyy] <- 5
  ...
  flankerDat$flanker_rt <- gsub("\\[", "", flankerDat$task1_stim_keyResp.rt) # removes the bracket
  flankerDat$flanker_rt <- gsub("\\]", "", flankerDat$flanker_rt) # removes the bracket
  flankerDat$flanker_rt <- sub(",.*", "", flankerDat$flanker_rt) # removing the RT for the second response within the same trial.
  flankerDat$flanker_rt <- as.numeric(flankerDat$flanker_rt)
  ...
  for (flanker_sub in 1:nrow(flankerDat)){
   ...
   flankerDat$flanker_resp <- as.numeric( str_extract(flankerDat$task1_stim_keyResp.keys, '[[:digit:]]')) #extracts the first number (first response) and converts them to numeric


  # loop over all flanker trials.
  for (trial in 1:nrow(flankerDat)){
    current_trial_face <- flankerDat$straightFace[trial] # The face shown during the current flanker trial
    current_trial_congruency <- flankerDat$congruent[trial]
    current_trial_rt <- flankerDat$flanker_rt[trial]
    ...
    if (flankerDat$task_trial_loop.thisTrialN[trial] == 0){ # if the trial is the first in its block
      ...
      pre_trial_rt <- flankerDat$flanker_rt[trial - 1]
    ...
    if (flankerDat$task_trial_loop.thisTrialN[trial] == 31){ # if the trial is the last in its block
      ...
      post_trial_rt <- flankerDat$flanker_rt[trial + 1]
      post_trial_resp_nums <- flankerDat$number_of_responses[trial + 1]
    }

    if (is.na(flankerDat$flanker_resp[trial])){ # When no response made in a flanker task trial
    ...
    } else if (!is.na(flankerDat$flanker_resp[trial])){ # When a response made in a flanker task trial
    ...
    if (flankerDat$task_trial_loop.thisTrialN[trial] == 0){ # if the trial is the first in its block
      ...
      if (is.na(flankerDat$flanker_resp[trial - 1])){ # When no response made in a flanker task trial
        pre_trial_responded <- 0 # 0 = not responded; 1 = responded
        pre_trial_accuracy <- 0 # Accuracy is considered 0 when there is no response
      } else if (!is.na(flankerDat$flanker_resp[trial - 1 ])){ # When a response made in a flanker task trial
    ...
    # If there is a response and the RT is larger than 150 ms, that  flanker trial is considered legit.
  ...
  # Checking the imported data at this stage for errors
  # The surprise task has 192 new images. If the surprise dataframe does not have 192 new images, it means there is an error.
  # The surprise task requires a response at every trial. So, there must be an RT for every trial. If sum of NA values in that column is not 0, there is an error in the code.
  # The surprise answers are a number from 1 to 6
  stopifnot(sum(as.numeric(surprise_memory_dat$new)) == 192, sum(is.na(surprise_memory_dat$firstStim_sliderA1.rt)) == 0, surprise_memory_dat$firstStim_sliderA1.response %in% (1:6))
  # checking whether the code above has labeled the answers correctly or not.
  if (surprise_memory_dat$cb[1] == "A"){
    for (ttt in 1:nrow(surprise_memory_dat)){
      if (surprise_memory_dat$surp_response_id[ttt] == 1){ # old
        if (surprise_memory_dat$surp_response_conf[ttt] == 1){
          if (surprise_memory_dat$surp_response_conf_all[ttt] == 1){
            stopifnot(surprise_memory_dat$firstStim_sliderA1.response[ttt] == 4) # maybe old
          }
        } else if (surprise_memory_dat$surp_response_conf[ttt] == 3){
          if (surprise_memory_dat$surp_response_conf_all[ttt] == 3){
            stopifnot(surprise_memory_dat$firstStim_sliderA1.response[ttt] == 5) # probably old
          }
        } else if (surprise_memory_dat$surp_response_conf[ttt] == 5){
          if (surprise_memory_dat$surp_response_conf_all[ttt] == 5){
            stopifnot(surprise_memory_dat$firstStim_sliderA1.response[ttt] == 6) # definitely old
          }
        }
      } else if (surprise_memory_dat$surp_response_id[ttt] == -1){ # new
        if (surprise_memory_dat$surp_response_conf[ttt] == 1){
          if (surprise_memory_dat$surp_response_conf_all[ttt] == -1){
            stopifnot(surprise_memory_dat$firstStim_sliderA1.response[ttt] == 3) # maybe new
          }
        } else if (surprise_memory_dat$surp_response_conf[ttt] == 3){
          if (surprise_memory_dat$surp_response_conf_all[ttt] == -3){
            stopifnot(surprise_memory_dat$firstStim_sliderA1.response[ttt] == 2) # probably new
          }
        } else if (surprise_memory_dat$surp_response_conf[ttt] == 5){
          if (surprise_memory_dat$surp_response_conf_all[ttt] == -5){
            stopifnot(surprise_memory_dat$firstStim_sliderA1.response[ttt] == 1) # Definitely new
          }
        }
      }
    }
  } else if (surprise_memory_dat$cb[1] == "B"){
    for (ttt in 1:nrow(surprise_memory_dat)){
      if (surprise_memory_dat$surp_response_id[ttt] == 1){ # old
        if (surprise_memory_dat$surp_response_conf[ttt] == 1){
          if (surprise_memory_dat$surp_response_conf_all[ttt] == 1){
            stopifnot(surprise_memory_dat$firstStim_sliderA1.response[ttt] == 3) # maybe old
          }
        } else if (surprise_memory_dat$surp_response_conf[ttt] == 3){
          if (surprise_memory_dat$surp_response_conf_all[ttt] == 3){
            stopifnot(surprise_memory_dat$firstStim_sliderA1.response[ttt] == 2) # probably old
          }
        } else if (surprise_memory_dat$surp_response_conf[ttt] == 5){
          if (surprise_memory_dat$surp_response_conf_all[ttt] == 5){
            stopifnot(surprise_memory_dat$firstStim_sliderA1.response[ttt] == 1) # definitely old
          }
        }
        ...
        if (surprise_memory_dat$surp_response_conf[ttt] == 1){
          if (surprise_memory_dat$surp_response_conf_all[ttt] == -1){
            stopifnot(surprise_memory_dat$firstStim_sliderA1.response[ttt] == 4) # maybe new
          }
        } else if (surprise_memory_dat$surp_response_conf[ttt] == 3){
          if (surprise_memory_dat$surp_response_conf_all[ttt] == -3){
            stopifnot(surprise_memory_dat$firstStim_sliderA1.response[ttt] == 5) # probably new
          }
        } else if (surprise_memory_dat$surp_response_conf[ttt] == 5){
          if (surprise_memory_dat$surp_response_conf_all[ttt] == -5){
            stopifnot(surprise_memory_dat$firstStim_sliderA1.response[ttt] == 6) # Definitely new
          }
        }
      }
    }
  ...
  # Minimum Surprise RT threshold that will be used to identify too fast trials.
  min_possible_rt <- 1 # in seconds

  # Identify trials to keep and count too fast trials
  surprise_df$keep_surp_memory_trial_based_on_subjectLevel_rt <-
    ifelse(surprise_df$memory_surp_rt >= min_possible_rt, 1, 0)

  surprise_df$num_tooFast_surpTrials <- sum(surprise_df$memory_surp_rt < min_possible_rt, na.rm = TRUE)



  # Adding a column that tells to which surprise block a given surp trial belongs.
  surprise_df$surp_block_num <- rep(1:8, each = 72)
  ...
  # 2. If a person has chosen the same response for at least 12 times in a row in a given surprise block, that block will be marked for exclusion.
  ...
  for (surp_block in 1:8){
    ...
    percent_of_max_surp_response <- (max(group_size(group_by(selected_rows,how_confident_all)))/sum(group_size(group_by(selected_rows,how_confident_all)))) * 100 # Percentage of the most chosen response in this block
    ...
    if (percent_of_max_surp_response >= 90){
      ...
      # 2. If a person has chosen the same response for at least 12 times in a row in a given
      # surprise block, that block will be marked for exclusion.

      for (surprise_block_trial in 1:(nrow(selected_rows) - 11)){
        the_same_surp_answer_counter <- 1
        ref_surp_answer <- selected_rows$how_confident_all[surprise_block_trial]
        for (ggg in 1:11){
          following_surp_answer <- selected_rows$how_confident_all[surprise_block_trial + ggg]
          ...
          # Check to see if the same answer counter is at 12.
