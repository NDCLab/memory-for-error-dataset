  ...
  if (nrow(incong_errorDat) >= outlier_num){ #for when the participant has outlier_num or more than outlier_num incongruent errors
    ...
    if (nrow(incong_errorDat) != 0){
      ...
      for (Jafa in 1:nrow(incong_errorDat)){
        ...
        identified_old_correctly <- ifelse (temp_for_surp$firstStim_sliderA1.response == 'Old', 1, 0)
    ...
    for (foil_num in 1:nrow(surpDat)){
      if (surpDat$new[foil_num] == 1){
        if (surpDat$firstStim_sliderA1.response[foil_num] == 'New'){
          num_foilFaces_reported_new <- num_foilFaces_reported_new + 1
        } else if (surpDat$firstStim_sliderA1.response[foil_num] == 'Old'){
    ...
    if (!is.null(post_error_faces)){
      ...
      for (zaman2 in 1:nrow(post_error_faces)){
        ...
        temp_for_surp <- filter(surpDat, surpriseFaces == temp_face)
        identified_old_correctly <- ifelse (temp_for_surp$firstStim_sliderA1.response == 'Old', 1, 0) #returns 1 when participant correctly identifies the face as OLD!
        if (identified_old_correctly == 1){
    ...
    percent_mainDat[nrow(percent_mainDat) + 1,] <-c(id, congAcc, incongAcc, congCorr_meanRT, incongCorr_meanRT, congCorr_logMeanRT,
                                                    ...
                                                    percent_incong_corrFaces_reported_new, percent_foilFaces_reported_new,
...
ggplot(percent_mainDat, aes(x=d_prime_error_minus_correct, y=scaared_b_scrdSoc_s1_r1_e1)) + geom_point(size = 4) + geom_abline(slope = coef(lm_for_cor_fit_line)[["d_prime_error_minus_correct"]],intercept = coef(lm_for_cor_fit_line)[["(Intercept)"]])
...
ggplot(percent_mainDat, aes(x=memoryBias_score, y=d_prime_error_minus_correct)) + geom_point(size = 4) + geom_abline(slope = coef(lm_for_cor_fit_line)[["memoryBias_score"]],intercept = coef(lm_for_cor_fit_line)[["(Intercept)"]])
