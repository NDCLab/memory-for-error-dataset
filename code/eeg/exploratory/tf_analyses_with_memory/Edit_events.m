...
% Last edit: June 10, 2024 (Aim: adding memory performance to the condition
% column)
...
data_location = [main_dir filesep 'derivatives' filesep 'preprocessed' filesep 'eeg' filesep 'added_memory'];

% Where you would like your epoched and newly labeled data saved
save_location = [main_dir filesep 'derivatives' filesep 'preprocessed' filesep 'eeg' filesep 'relabeled_for_TF_with_memory'];
...
for t=1:length(EEG.event)
    ...
    memoryAcc = {EEG.event.memoryAcc};
    
    % Selecting specific variables for this trial
    vars_to_join = {eventType{t}, num2str(congruency{t}), num2str(accuracy{t}), memoryAcc{t} };
    EEG.event(t).Condition = strjoin(vars_to_join, '_');
    % Kia: outputs will b like 'resp_i_0_remembered','resp_i_1_remembered', 'resp_c_1_remembered',
    % 'stim_i_0_remembered', 'stim_i_1_remembered','stim_c_1_remembered', etc.!
