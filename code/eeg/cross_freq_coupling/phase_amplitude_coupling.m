...
for sub=1: 2%length(subject_list)
    ...
    if size(cond1_data, 3) >= min_trialNum_perCondition
        if size(cond2_data, 3) >= min_trialNum_perCondition
            ...
            for e=1:size(cond1_data,3) % begin loop through epochs
                ...
                %elecs4PAC)
                for i=1:length(elecs4PAC)
                    elecs4PAC_idx (i)= find(strcmp({cond1_EEG.chanlocs.labels}, elecs4PAC{i}));
                end
                %do any of these electrodes have NaNs in them?
                if min(sum(isnan(cond1_EEG.data(elecs4PAC_idx,:,e)))) > 0
                    %if so, add that epoch to vector of epochs to be removed
                    epochs2remove = [epochs2remove e];
                else
                    %do nothing
            ...
            % keep only channels of interest
            cond1_data = cond1_data(elecs4PAC_idx,:,:); % channel x time/data points x trials
            ...
            for e=1:size(cond2_data,3) % begin loop through epochs
                ...
                %elecs4PAC)
                for i=1:length(elecs4PAC)
                    elecs4PAC_idx (i)= find(strcmp({cond2_EEG.chanlocs.labels}, elecs4PAC{i}));
                end
                %do any of these electrodes have NaNs in them?
                if min(sum(isnan(cond2_EEG.data(elecs4PAC_idx,:,e)))) > 0
                    %if so, add that epoch to vector of epochs to be removed
                    epochs2remove = [epochs2remove e];
                else
                    %do nothing
            ...
            % keep only channels of interest
            cond2_data = cond2_data(elecs4PAC_idx,:,:); % channel x time/data points x trials
                ...
                for lfb_counter = 1:length(lower_freq_band_indices)
                    ...
                    for ufb_counter = 1:length(upper_freq_band_indices)
                        ...
                        for elecs4Amp_idx = 1:length(elecs4Amp)
                            ...
                            for elecs4PAC_idx = 1:numel(elecs4PAC)
                                if strcmp(elecs4PAC{elecs4PAC_idx}, tempElec_from_elecs4Amp) % find the index of this iteration's electrode in elecs4PAC.
                                    freq4Power_elec_idx = elecs4PAC_idx; % the index of this iteration's electrode in elecs4PAC
