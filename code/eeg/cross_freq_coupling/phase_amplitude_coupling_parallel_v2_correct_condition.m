...
% This code just computes correct condition (i.e., cond2)
clear;
clc;
...
subject= {subnum.name};
already_processed = ["170040_mfe_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200001_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200002_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200003_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200004_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200005_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200006_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200007_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200011_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200012_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200014_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200015_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200016_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200017_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200018_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200020_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200021_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200022_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200024_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200025_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200026_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200027_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200028_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200029_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200031_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200033_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200034_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200046_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200048_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200053_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200062_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200076_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200079_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200084_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set"]; % add the name of the subjects already processed here. Example: ["sub-200100_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200088_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set"]
...
parfor sub=1:length(subject_list)
    ...
    fprintf('\n\n\n*** In the correct condition: processing subject %d (%s) ***\n\n\n', sub, subject);
    subjStart = tic;
    ...
    if size(cond1_data, 3) >= min_trialNum_perCondition
        if size(cond2_data, 3) >= min_trialNum_perCondition
            ...
            for subsample = 1:numSubsamples
                ...
                current_subject_data_cond2 = table;

                subtrials_cond2=[]; data_cond2_temp=[]; 

                
                % Get indices of trials from this condition
                subtrials_cond2 = randsample(1:size(cond2_data,3),(numPulledTrials),false); % random sampling without replacement
                % Index into those trials and pull them out
                data_cond2_temp = cond2_data(:,:,subtrials_cond2);
                
                %% Condition 2: 
                %% Run wavelet convolution
                timefreqs = zeros(length(frex), epoch_data_length, size(data_cond2_temp, 3)); % freqs x time points x trials
                phase_data = zeros(length(frex), epoch_data_length, size(data_cond2_temp, 3)); % freqs x time points x trials
                timefreqs_strial_ch = zeros(length(frex), epoch_data_length, size(data_cond2_temp, 3), size(data_cond2_temp, 1)); % freqs x time points x trials x channels
                phase_data_strial_ch = zeros(length(frex), epoch_data_length, size(data_cond2_temp, 3), size(data_cond2_temp, 1)); % freqs x time points x trials x channels
                for ch=1:size(data_cond2_temp, 1) % Loop through all channels
                    for fi=1:length(frex) % loop through all frequencies
                        trial_conv = zeros(epoch_data_length, size(data_cond2_temp, 3));
                        ...
                        trial_temppow = zeros(size(data_cond2_temp, 2), size(data_cond2_temp, 3));
                        for trl=1:size(data_cond2_temp, 3)
                            temp_data = fft(squeeze(data_cond2_temp(ch,:,trl)), conv_output_length);
                                ...
                                if strcmp(elecs4PAC{iii}, tempElec_from_elecs4Amp) % find the index of this iteration's electrode in elecs4PAC.
                                    ...
                                    for timei=1:length(times4PAC)
                                        ...
                                        for iter_counter=1:num_perm_iter
                                            ...
                                            random_timepoint = randsample(round(cfc_time_window_idx*.8),size(data_cond2_temp,3),1)+round(cfc_time_window_idx*.1);
                                            for triali=1:size(data_cond2_temp,3)
                                        ...
                                        is_this_error_resp_condition = 0; % error is 1 and correct is 0!
                                        ...
                                        current_subject_data_cond2 = [current_subject_data_cond2; new_row];
                ...
                current_output_name_cond2 = append(save_location, 'sub-', string(subject_id), 'cond2_subsample_', string(subsample),'_PACz', '.csv');
                writetable(current_subject_data_cond2, current_output_name_cond2);
                %% END Condition 2
            end % end of the loop for subsampling
            
            subjEnd = toc(subjStart);
            fprintf('Processing correct condition (cond2) completed for subject %s in %d hours %.3f minutes, continuing.\n', subject_id, floor(subjEnd/3600), rem(subjEnd,3600)/60);
