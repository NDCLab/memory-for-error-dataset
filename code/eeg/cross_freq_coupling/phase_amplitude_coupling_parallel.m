...
%fprintf('SLURM CPUS ON NODE: %f', str2num(getenv('SLURM_CPUS_ON_NODE')));
%fprintf('maxNumCompThreads: %f', maxNumCompThreads);
...
% Save Data Location
save_location = [main_dir filesep 'derivatives' filesep 'preprocessed' filesep 'eeg' filesep 'cfc' filesep 'main' filesep ];
...
elecs4Amp = {'1', '6', '9', '22', '24', '39', '42', '53', '55'}; % MFC
...
elecs4PAC = {'1', '6', '9', '22', '24', '39', '42', '53', '55'};
...
% 9. Choose the time ponits for which you want to compute PAC?
times4PAC = 100:100:500;
...
subject= {subnum.name};
%subject = {'sub-200005_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set', 'sub-200006_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set', 'sub-200007_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set', 'sub-200008_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set'};
%subject = {'sub-200008_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set'}
already_processed = ["sub-200100_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200088_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200091_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200095_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200045_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200010_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200013_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200032_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200019_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200060_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200009_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200001_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200002_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200003_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200004_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200005_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200006_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200007_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200008_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "170040_mfe_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200052_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200053_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set", "sub-200028_mfe_b_eeg-flanker_s1-r1-e1_processed_data.set"];
...
parfor sub=1:length(subject_list)
    ...
    if length(subject) == 0 %skip over already-processed subs
        continue
    end
    ...
    %for i=1:lels
    %    elecs4PAC_idx (i)= find(strcmp({EEG.chanlocs.labels}, elecs4PAC{i}));
    %end
    elecs4PAC_idx = [1, 6, 9, 22, 24, 39, 42, 53, 55];
    ...
    if size(cond1_data, 3) >= min_trialNum_perCondition
        if size(cond2_data, 3) >= min_trialNum_perCondition
            ...
            all_data(:,:,unique(epochs2remove))=[];
            % keep only channels of interest
            all_data = all_data(elecs4PAC_idx,:,:); % channel x time/data points x trials
            ...
            %% Prepare condition 2 data for wavelet
            % Loop through epochs and remove the epochs that are NaN
            epochs2remove = [];
            %for i=1:lels
            %    elecs4PAC_idx_cond2 (i)= find(strcmp({cond2_EEG.chanlocs.labels}, elecs4PAC{i}));
            %end
            for e=1:size(cond2_data,3) % begin loop through epochs
                %loop through and find indices of the electrodes of interest (i.e.,
                %elecs4PAC)
                %do any of these electrodes have NaNs in them?
                if min(sum(isnan(cond2_EEG.data(elecs4PAC_idx_cond2,:,e)))) > 0
                    %if so, add that epoch to vector of epochs to be removed
                    epochs2remove = [epochs2remove e];
                else
                    %do nothing
            ...
            % remove all epochs with NaNs for channels of interest
            cond2_data(:,:,unique(epochs2remove))=[];
            % keep only channels of interest
            cond2_data = cond2_data(elecs4PAC_idx_cond2,:,:); % channel x time/data points x trials
            ...
            variable_names = {'subject_id', 'subsample_iteration', 'lower_frequency', 'upper_frequency', 'lower_frequency_channel', 'upper_frequency_channel', 'time_point', 'PACz'};
            % variable_types = {'string','double','double', 'double', 'string', 'string', 'double', 'double'};
            current_subject_data = table;
            ...
            for subsample = 1:numSubsamples
                ...
                % Creating the following empty arrays so that we can reuse them in each iteration
                % of the loop.
                subtrials_cond1=[]; subtrials_cond2=[]; data_cond1_temp=[]; data_cond2_temp=[]; data_all_conds = [];

                % Get indices of trials from this condition
                subtrials_cond1 = randsample(1:size(cond1_data,3),(numPulledTrials/2),false); % random sampling without replacement
                % Index into those trials and pull them out
                data_cond1_temp = cond1_data(:,:,subtrials_cond1);

                % Get indices of trials from this condition
                subtrials_cond2 = randsample(1:size(cond2_data,3),(numPulledTrials/2),false); % random sampling without replacement
                % Index into those trials and pull them out
                data_cond2_temp = cond2_data(:,:,subtrials_cond2);
                % concatenating the data from both conditions along the third dimension (trials)
                data_all_conds = cat(3, data_cond1_temp, data_cond2_temp); % channels x time/data points x trials

                %% Run wavelet convolution
                timefreqs = zeros(length(frex), epoch_data_length, size(data_all_conds, 3)); % freqs x time points x trials
                phase_data = zeros(length(frex), epoch_data_length, size(data_all_conds, 3)); % freqs x time points x trials
                timefreqs_strial_ch = zeros(length(frex), epoch_data_length, size(data_all_conds, 3), size(data_all_conds, 1)); % freqs x time points x trials x channels
                phase_data_strial_ch = zeros(length(frex), epoch_data_length, size(data_all_conds, 3), size(data_all_conds, 1)); % freqs x time points x trials x channels
                for ch=1:size(data_all_conds, 1) % Loop through all channels
                    for fi=1:length(frex) % loop through all frequencies
                        trial_conv = zeros(epoch_data_length, size(data_all_conds, 3));
                        ...
                        trial_temppow = zeros(size(data_all_conds, 2), size(data_all_conds, 3));
                        for trl=1:size(data_all_conds, 3)
                            temp_data = fft(squeeze(data_all_conds(ch,:,trl)), conv_output_length);
                                ...
                                if strcmp(elecs4PAC{iii}, tempElec_from_elecs4Amp) % find the index of this iteration's electrode in elecs4PAC.
                                    ...
                                    for timei=1:length(times4PAC)
                                        ...
                                        for iter_counter=1:num_perm_iter
                                            ...
                                            random_timepoint = randsample(round(cfc_time_window_idx*.8),size(data_all_conds,3),1)+round(cfc_time_window_idx*.1);
                                            for triali=1:size(data_all_conds,3)
                                        ...
                                        new_row = table(string(subject_id), subsample, current_freq4Phase, current_freq4Power, elecs4PAC(freq4Phase_elec_idx), elecs4PAC(freq4Power_elec_idx), times4PAC(timei), PACz, 'VariableNames', variable_names);
                                        % Append the new row to the current
                                        % subject's data table
                                        current_subject_data = [current_subject_data; new_row];
            ...
            current_output_name = append(save_location, 'sub-', string(subject_id), '_all_subsamples_PACz', '.csv');
            writetable(current_subject_data, current_output_name);
            subjEnd = toc(subjStart);
            fprintf('Processing completed for subject %s in %d hours %.3f minutes, continuing.\n', subject_id, floor(subjEnd/3600), rem(subjEnd,3600)/60);
        ...
        disp(['error seen in subject ' subject_id]);
    end
