...
parfor sub=1:length(subject_list)
    ...
    if size(cond1_data, 3) >= min_trialNum_perCondition
        if size(cond2_data, 3) >= min_trialNum_perCondition
            ...
            current_output_name_cond1 = append(save_location, 'sub-', string(subject_id), 'cond1_all_subsamples_PACz', '.csv');
            current_output_name_cond2 = append(save_location, 'sub-', string(subject_id), 'cond2_all_subsamples_PACz', '.csv');
